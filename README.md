...
1. New → Blueprint, pick this repo. Render creates both services and the
   ...
   `sync: false` isn't allowed inside an env group).
2. Fill the remaining `sync: false` secrets: on `sidecar`, `SIDECAR_APNS_TOPIC`,
   `SIDECAR_OBA_API_KEY`, and `SIDECAR_PIRATE_WEATHER_KEY`; on `gorush`, the
   three `GORUSH_IOS_*` credential values.
...
not a path or base64 encoding of it.
