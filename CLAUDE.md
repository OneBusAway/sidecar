...
**Wiring lives only in `cmd/sidecar/main.go`**: open store → `Migrate()` → build `Deps` → start background loops (regions directory sync, push-registration prune, alarm scheduler, alert push dispatcher, ghost bus snapshot enrichment, Live Activity updater) → serve. Every loop is a per-tick function (`CheckAll`, `RunOnce`, `Sync`, `Prune`) driven by one `lease.Runner` (`internal/lease`): each loop holds a named row in the `leases` table, renewed every poll, so with a shared database only one process runs a given loop at a time and a survivor takes over within the TTL. Domain packages own no tickers — cadence lives in `main.go`. `compose.yaml` (local) and `render.yaml` (Render) mirror each other on ports 8080 (sidecar) and 8088 (gorush).
...
- **Two more sqlc traps, same silent shape:** keep `queries/*.sql` comments ASCII-only — sqlc renumbers `sqlc.arg()` by byte offset into the statement text, so a multi-byte rune (a `§`, an em dash) in a preceding comment shifts the offsets and emits garbage SQL; cite the design spec as "spec section N" instead. And sqlc does not extract a parameter written inside an `IN (...)` list: `sqlc.arg(x) IN ('a','b')` compiles, diffs clean, and hands the literal text `sqlc.arg(x)` to the driver — spell such guards out as OR comparisons. The same goes for a parameter inside `ON CONFLICT ... DO UPDATE ... WHERE` — put the condition in an `INSERT ... SELECT ... WHERE NOT EXISTS` guard instead (see `queries/leases.sql`).
- **Rate limiters key on the TCP peer address by default**, deliberately ignoring `X-Forwarded-For`. Don't "fix" this; the only sanctioned alternative is the opt-in `SIDECAR_TRUSTED_PROXY` (`internal/clientip`), which reads a header the proxy is known to overwrite. The README's Deployment section documents the proxy requirement.
- The alarm scheduler sweeps `ListDue(now)`, not `List`: a `Wait` defers the alarm's next check halfway to its fire window (`alarms.check_after`; a deferral shorter than `MinDeferral` is skipped, so alarms within ~4 minutes of their window are checked every minute). Keep `List` returning every row — the admin API and tests read it.
