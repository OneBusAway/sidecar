...
- The alarm scheduler sweeps `ListDue(now)`, not `List`: a `Wait` defers the alarm's next check halfway to its fire window (`alarms.check_after`, floor `MinDeferral`). Keep `List` returning every row — the admin API and tests read it.
