...
func runImport(ctx context.Context, stdout io.Writer, store *sqlite.Store, now time.Time, args []string) error {
	...
	fmt.Fprintf(stdout, "  alerts             %d added, %d already present\n", sum.Alerts, sum.AlertsSkipped)
	fmt.Fprintf(stdout, "  studies            %d added, %d already present\n", sum.Studies, sum.StudiesSkipped)
	fmt.Fprintf(stdout, "  surveys            %d added, %d already present\n", sum.Surveys, sum.SurveysSkipped)
	fmt.Fprintf(stdout, "  questions          %d added, %d already present\n", sum.Questions, sum.QuestionsSkipped)
	fmt.Fprintf(stdout, "  survey responses   %d added, %d already present\n", sum.SurveyResponses, sum.SurveyResponsesSkipped)
	fmt.Fprintf(stdout, "  push registrations %d added, %d already present\n", sum.PushRegistrations, sum.PushRegistrationsSkipped)
	fmt.Fprintf(stdout, "  ghost bus reports  %d added, %d already present\n", sum.GhostBusReports, sum.GhostBusReportsSkipped)
	return nil
}
