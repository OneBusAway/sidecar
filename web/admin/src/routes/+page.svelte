<script lang="ts">
	...
	// '' is "all regions". Never 0 -- region 0 is Tampa Bay, so a numeric
	// sentinel would make selecting Tampa mean "no filter".
	let regionFilter = $state(ALL_REGIONS);

	const shown = $derived(filterByRegion(data.alerts, regionFilter));
</script>

<div class="head">
	<h1>Alerts</h1>
	<a class="button" href={resolve('/alerts/new')}>New alert</a>
</div>
...
{#if shown.length === 0}
	<p class="empty">
		{#if data.alerts.length === 0}
			No alerts yet. <a href={resolve('/alerts/new')}>Create one</a>.
		{:else}
	...
	<table>
		...
		<tbody>
			{#each shown as a (a.id)}
				{@const region = regionById(data.regions, a.region_id)}
				<tr>
					<td>
						<a href={resolve('/alerts/[id]', { id: String(a.id) })}>
							{a.header}
						</a>
					</td>
					<td>{regionName(data.regions, a.region_id)}</td>
					<td>
						{formatInstantForRegion(a.start_time, region?.timezone ?? '')}
						{#if region?.timezone}
							<span class="zone">{region.timezone}</span>
						{:else}
							<!--
